...
class User:
    ...
    def get_by_email(cls, formulario):
        ...
        query = "SELECT * FROM users WHERE email = %(email)s"
        result = connectToMySQL('muro_privado').query_db(query, formulario)# SELECT regresa siempre una lista, si no existe es una lista vacia, el usuario 0 es el que inicia sesion
    ...
    def get_all(cls):
        ...
        results = connectToMySQL('muro_privado').query_db(query) #regresa una lista de disccionarios
        #results = [
        # {id:1, first_name: elena, last_name.....}
        # {id:2, first_name: juana, last_name.....}]
        users = []
        for user in results:
            users.append(cls(user))#1.- cls(user) crea una instancia en base al diccionario 2.- agrega la instancia a la lista users
        return users
